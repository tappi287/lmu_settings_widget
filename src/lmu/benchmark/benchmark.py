...
from lmu.directInputKeySend import rfKeycodeToDIK
from lmu.globals import GAME_EXECUTABLE
from lmu.preset.preset import PresetType, GraphicsPreset
...
from lmu.preset.settings_model import BenchmarkControllerJsonSettings, VideoSettings
from lmu.process import RunProcess
from lmu.benchmark.benchmark_utils import create_benchmark_commands, create_quit_commands, BenchmarkRun, BenchmarkQueue
from lmu.benchmark.fpsvr import FpsVR
from lmu.rf2connect import RfactorConnect, RfactorState
from lmu.rf2events import StartBenchmarkEvent, RecordBenchmarkEvent, RfactorQuitEvent
from lmu.rf2events import RfactorStatusEvent, BenchmarkProgressEvent
...
class RfactorBenchmark:
    ...
    def event_loop(self):
        ...
        if self.recording and self.start_time > 0.0:
            ...
            percent = round(
                max(0.0, self.benchmark_length + 10.0 - remaining) * 100 / max(1.0, self.benchmark_length + 10.0)
            )
    ...
    def _prepare_benchmark_run(self) -> bool:
        ...
        logging.info(
            "Prepared Benchmark Run: %s length %s recording delay %s replay %s " "launch method %s with Presets: %s",
    ...
    def _update_controller_assignment(self) -> bool:
        ...
        if hasattr(self.rf.options, key):
            ...
            for con_key in (self.ai_key, self.fps_key):
                ...
                if o and isinstance(o.value, int):
                    if con_key == self.ai_key:
                        self.ai_dik = rfKeycodeToDIK(o.value)
                    elif con_key == self.fps_key:
                        self.fps_dik = rfKeycodeToDIK(o.value)
                    result = True
                    logging.debug("Read controller assignment %s: %s", con_key, rfKeycodeToDIK(o.value))
                else:
                    result = False
            return result
        return False
    ...
    def _create_result(self):
        ...
        for preset_type in (PresetType.session, PresetType.graphics):
            ...
            _, selected_preset = load_presets_from_dir(
                get_user_presets_dir(), preset_type, selected_preset_name=selected_preset_name
            )
    ...
    def start_present_mon_logging(self) -> bool:
        ...
        cmd = [
            AppSettings.present_mon_bin.as_posix(),
            "-process_name",
            GAME_EXECUTABLE,
            "-output_file",
            str(WindowsPath(self.result_file)),
            "-timed",
            str(int(self.benchmark_length)),
            "-no_top",
            "-qpc_time",
