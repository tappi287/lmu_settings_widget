...
def create_shortcut(executable_path: Path, working_directory: Path, shortcut_location: Path, arguments: str = ""):
...
def subprocess_args(include_stdout=True, cwd=None):
    ...
    ret.update({"stdin": sp.PIPE, "stderr": sp.PIPE, "startupinfo": si, "env": env, "cwd": cwd})
    return ret
