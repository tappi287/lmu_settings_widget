...
def get_presets(preset_type: int):
    ...
    presets, selected_preset = load_presets_from_dir(
        get_user_presets_dir(), preset_type, current_preset, selected_preset_name
    )
    if selected_preset:
    ...
    if selected_preset and selected_preset != current_preset:
        ...
        logging.debug(
            'Resetting selected Preset to "Current Preset" from %s because settings differ '
            "from actual rFactor 2 settings.",
            preset_changed,
        )
    ...
    return json.dumps(
        {
            "result": True,
            "presets": [p.to_js() for p in presets],
...
def get_user_presets_dir_web():
    logging.info("Providing User Presets Dir to FrontEnd: %s", get_user_presets_dir().as_posix())
    return get_user_presets_dir().as_posix()
    ...
    if AppSettings.replay_playing:
        if AppSettings.replay_preset != "":
            ...
            _, selected_preset = load_presets_from_dir(
                get_user_presets_dir(), PresetType.graphics, selected_preset_name=selected_preset_name
            )
