...
def get_content():
    if not AppSettings.content:
        return json.dumps(
            {
                "result": False,
                "msg": "No rFactor 2 content listed. Start the game at least once to " "populate the content list.",
            }
        )
    content = AppSettings.content

    # -- Handle post v1124 Series entries
    for series in content.get("series", list()):
    ...
    for track in content.get("tracks", list()):
        ...
        content["layout"].append(
            {"id": layout, "location": location, "shortName": layout, "name": layout, "track_id": track.get("id")}
        )
    ...
    logging.debug(
        "Collected content dict for FrontEnd:"
        f'Series: {len(content.get("series"))} Tracks: {len(content.get("tracks"))} '
        f'Location: {len(content.get("location"))} Layouts: {len(content.get("layout"))} '
        f'Manufacturer: {len(content.get("manufacturer"))} Models: {len(content.get("model"))} '
        f'Cars: {len(content.get("cars"))}'
    )
